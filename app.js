...
let chart = null;
let pendingReturn = null; // { view: 'block'|'practice', data: {...} } — set when subject is quick-added mid-form
...
function renderDashboard() {
  ...
  tallyEl.innerHTML = "";
  if (streak === 0) {
    tallyEl.innerHTML = `<span class="flame" style="opacity:0.35;filter:none;animation:none;">🕯️</span>`;
  } else {
    const flameCount = Math.min(1 + Math.floor(streak / 3), 6);
    for (let i = 0; i < flameCount; i++) {
      const f = document.createElement("span");
      f.className = "flame";
      f.textContent = "🔥";
      tallyEl.appendChild(f);
    }
  }
...
async function saveSubject(e) {
  ...
  try {
    ...
    if (pendingReturn) {
      ...
      pendingReturn = null;
      if (pr.view === "block") {
        openBlockModal(pr.data.id || null);
        document.getElementById("blockDate").value = pr.data.date;
        document.getElementById("blockStart").value = pr.data.start;
        document.getElementById("blockEnd").value = pr.data.end;
        document.getElementById("blockTopic").value = pr.data.topic;
        document.getElementById("blockNotes").value = pr.data.notes;
        document.getElementById("blockSubject").value = newId;
      } else if (pr.view === "practice") {
        document.getElementById("practiceForm").reset();
        openModal("practice");
        document.getElementById("practiceDate").value = pr.data.date;
        document.getElementById("practiceAttempted").value = pr.data.attempted;
        document.getElementById("practiceCorrect").value = pr.data.correct;
        document.getElementById("practiceSubject").value = newId;
      }
    }
  } catch (err) {
    console.error(err);
...
function initModals() {
  ...
  document.querySelectorAll("[data-quick-add-subject]").forEach(btn => {
    btn.addEventListener("click", () => {
      const returnTo = btn.dataset.quickAddSubject;
      if (returnTo === "block") {
        pendingReturn = {
          view: "block",
          data: {
            ...
            topic: document.getElementById("blockTopic").value,
        ...
        pendingReturn = {
          ...
          data: {
            date: document.getElementById("practiceDate").value,
            attempted: document.getElementById("practiceAttempted").value,
            correct: document.getElementById("practiceCorrect").value
          }
        };
        closeModal("practice");
      }
